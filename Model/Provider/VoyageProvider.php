...
class VoyageProvider implements EmbeddingProviderInterface
{
    ...
    public function embed(array $texts, int $timeoutMs = 30000, string $inputType = self::TYPE_DOCUMENT): array
    {
        ...
        try {
            $curl->post(self::ENDPOINT, $this->json->serialize([
                ...
                'output_dimension' => $this->config->getDimensions(),
                'input_type'       => $inputType,
            ]));
        } catch (\Throwable $e) {
            throw new ProviderException('Voyage embeddings transport error: ' . $e->getMessage(), 0, $e);
        }
        ...
        if ($curl->getStatus() === 429) {
            ...
            throw new RateLimitException(
                sprintf('%s embeddings HTTP 429 (rate limited): %s', 'Voyage', substr($body, 0, 300)),
                $retryAfter
            );
        }
        ...
        try {
            ...
            throw new ProviderException('Voyage embeddings: invalid JSON response', 0, $e);
        }

        if (!isset($response['data']) || count($response['data']) !== count($texts)) {
            throw new ProviderException('Voyage embeddings: response count mismatch');
        }

        foreach ($response['data'] as $row) {
            if (!isset($row['index'], $row['embedding']) || !is_array($row['embedding'])) {
                throw new ProviderException('Voyage embeddings: malformed response row');
            }
        }
